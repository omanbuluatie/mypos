...
class MonthlySalesReportScreen extends ConsumerStatefulWidget {
  ...
  @override
  _MonthlySalesReportScreenState createState() => _MonthlySalesReportScreenState();
}
...
class _MonthlySalesReportScreenState extends ConsumerState<MonthlySalesReportScreen> {
  ...
  Future<void> _loadSales() async {
    sales = await DatabaseHelper().getSalesByMonth(selectedYear, selectedMonth);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Laporan Penjualan Bulanan'),
      ),
      body: Padding(
        ...
        child: Column(
          children: [
            Row(
              children: [
                Text('Bulan/Tahun: $selectedMonth/$selectedYear'),
                SizedBox(width: 16),
                ElevatedButton(
